(function () {
  ...
  function advertisement() {
    ...
    slot.innerHTML='<span>광고</span><ins class="adsbygoogle" style="display:block;width:100%;height:250px" data-ad-client="'+esc(config.ad.client)+'" data-ad-slot="'+esc(config.ad.slot)+'"></ins>';
    let requested=false;
    function request(){if(requested) return; requested=true;try{(window.adsbygoogle=window.adsbygoogle||[]).push({});track('reading_ad_request');}catch(e){slot.hidden=true;}}
    if(window.IntersectionObserver){const observer=new IntersectionObserver(entries=>{if(entries.some(e=>e.isIntersecting && e.intersectionRatio>=0.1)){request();observer.disconnect();}},{rootMargin:'0px',threshold:0.1});observer.observe(slot);}else request();
  }
  async function detail() {
    ...
    content.innerHTML='<p class="reading-lead">'+esc(fee(row))+'</p><p class="reading-summary">'+esc(row.ad||'주소 미등록')+'<br>운영시간: '+esc(row.tm||'미등록')+'</p>'+routes(row)+'<section><h2>무료 이용 조건</h2><p>'+esc(conditions(row))+'</p></section><aside id="reading-ad" aria-label="광고"></aside><section><h2>요금 기준</h2><dl>'+[
    ...
    ].map(([k,v])=>'<dt>'+k+'</dt><dd>'+esc(v)+'</dd>').join('')+'</dl><p>30분 환산 요금은 비교용입니다. 실제 결제액은 최소 부과 단위·할인·운영 규정에 따라 달라집니다. 미등록은 무료를 의미하지 않습니다.</p></section><section><h2>운영 정보</h2><dl>'+facts.map(([k,v])=>'<dt>'+k+'</dt><dd>'+esc(v)+'</dd>').join('')+'</dl>'+(row.tel?'<p>문의: <a href="tel:'+esc(row.tel)+'">'+esc(row.tel)+'</a></p>':'')+'</section><section><h2>주변 대안 비교</h2><p>이 주차장을 기준으로 가까운 곳의 요금과 무료 조건을 함께 확인하세요.</p><a class="reading-primary" href="/compare/?lat='+row.la+'&amp;lng='+row.lo+'&amp;name='+encodeURIComponent(row.nm)+'">이 주변 주차장 비교하기</a></section><p class="reading-note">자료: 공공데이터포털·서울 열린데이터광장. 운영정보는 현장 안내를 우선하며 실시간 빈자리는 제공하지 않습니다.</p>';
  }
  async function compare() {
    ...
    function paint(){let selected=rows.filter(r=>!document.querySelector('#compare-free').checked || r.fr); if(document.querySelector('#compare-sort').value==='price')selected.sort((a,b)=>(a.fr?0:a.p30||Infinity)-(b.fr?0:b.p30||Infinity)||a.d-b.d);
      const cards=selected.map(r=>'<article><h2><a href="'+esc(detailUrl(r))+'">'+esc(r.nm)+'</a></h2><p><strong>'+esc(fee(r))+'</strong> · 직선 '+r.d.toFixed(2)+'km</p><p>'+esc(r.ad)+'</p><p>'+esc(conditions(r))+'</p><p>운영시간: '+esc(r.tm||'미등록')+'</p><a class="reading-primary" href="'+esc(detailUrl(r))+'">요금·무료 조건 상세</a>'+routes(r)+'</article>'); document.querySelector('#compare-list').innerHTML=cards.slice(0,1).join('') || '<p>조건에 맞는 주차장이 없습니다. 지도에서 다른 지역을 찾아보세요.</p>'; document.querySelector('#compare-tail').innerHTML=cards.slice(1).join('');}
